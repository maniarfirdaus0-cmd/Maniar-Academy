...
const WHATSAPP_URL = `https://wa.me/${9374111023}?text=${WHATSAPP_MESSAGE}`;
...
export default function Footer({ onAdminClick }) {
  ...
  return (
    <footer id="contact" className="border-t border-white/5 bg-[#08080a]">
      <div className="mx-auto max-w-7xl px-6 py-16 lg:px-8">
        <div className="grid gap-12 lg:grid-cols-3">
          ...
          <div>
            ...
            >
              <span className="flex h-10 w-10 items-center justify-center rounded-xl bg-accentViolet text-sm font-bold text-white shadow-lg shadow-accentViolet/25">
                MA
              </span>
            ...
            <ul className="mt-4 grid grid-cols-2 gap-2 sm:grid-cols-1">
              {footerNav.map(({ label, href }) => (
                <li key={href}>
                  <a
                    href={href}
                    className="inline-block rounded-md px-1 py-1.5 text-sm text-slate-400 transition-all duration-300 hover:translate-x-1 hover:text-accentViolet-light"
                  >
                    {label}
                  </a>
                </li>
          ...
          <div>
            ...
            <p className="mt-2 text-sm text-slate-400">
              Follow us on social media or message directly on WhatsApp.
            </p>
            <div className="mt-5 flex flex-wrap gap-3">
              {socialLinks.map(({ label, href, hover, icon }) => (
                <a
                  key={label}
                  href={href}
                  target="_blank"
                  ...
                  aria-label={label}
                  className={`flex h-12 w-12 items-center justify-center rounded-xl border border-white/10 bg-white/5 text-slate-300 transition-all duration-300 ease-in-out hover:scale-110 ${hover}`}
                >
                  {icon}
                </a>
              ))}
        ...
        <div className="mt-12 flex flex-col items-center justify-between gap-4 border-t border-white/5 pt-8 sm:flex-row">
          <div className="flex flex-col sm:flex-row items-center gap-3">
            <p className="text-sm text-slate-500">
              &copy; {year} Maniar Academy. All rights reserved.
            </p>
            <button
              onClick={onAdminClick}
              className="text-xs text-slate-600 hover:text-accentViolet-light font-medium cursor-pointer transition-colors"
            >
              • Admin Dashboard
            </button>
          </div>
          <a
            ...
            target="_blank"
            rel="noopener noreferrer"
            className="inline-flex items-center gap-2.5 rounded-lg bg-green-600 px-5 py-2.5 text-sm font-semibold text-white shadow-lg shadow-green-600/10 transition-all duration-300 ease-in-out hover:scale-105 hover:bg-green-500"
          >
